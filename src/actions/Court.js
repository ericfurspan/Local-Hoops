  ...
  try {
    ...
    firebase
      ...
      .set({
        ...
        name: data.name,
        discovered_by: {
          // if court from 3rd source (i.e Google Places), use the displayName given from UI and the current users id
          displayName: data.discovered_by
            ? data.discovered_by.displayName
            : getState().currentUser.displayName,
...
export const getNearbyCourts = (coords, searchRadius = 15000) => async (dispatch, getState) => {
  try {
    ...
    const courtsRef = firebase.firestore().collection('courts');
    const courtsSnapshot = await courtsRef.get();
    const centerCoords = { latitude: coords.latitude, longitude: coords.longitude };
    let firestoreCourts = courtsSnapshot.docs.filter((doc) => {
      const courtCoords = { latitude: doc.data().coords.latitude, longitude: doc.data().coords.longitude };
      const isMatch = geolib.isPointInCircle(courtCoords, centerCoords, searchRadius);
      if (isMatch) {
        return true;
      } else {
        return false;
    ...
    if (res.error) {
      ...
      if (!firestoreCourtIds) {
        ...
        // some saved courts, make sure not to include any google courts that are already saved to firestore
        res.length > 0 &&
          res.forEach((court) => {
