...
class ViewFriend extends React.Component {
  ...
  fetchUserEvents = () => {
    this.unsubscribeUserEvents = firebase
      .firestore()
      .collection('events')
      .where('participants', 'array_contains', this.props.friend.uid)
      .onSnapshot(
        (eventsSnapshot) => {
          const events = [];
          let counter = 0;
  ...
  confirmRemove = (friend) => {
    AlertIOS.alert('Please Confirm', `Are you sure you want to unfollow ${friend.displayName}?`, [
      {
        text: 'Cancel',
      },
      {
  ...
  render() {
    ...
    if (this.state.events) {
      ...
      const eventsData = this.state.events.map((event, i) => {
        return {
          time: event.date,
          title: event.type,
      ...
      if (this.state.modalVisible) {
        modal = (
          <View>
            <StatusBar hidden />
            <EventModal setModalVisible={this.setModalVisible} event={this.state.selectedEvent} />
          </View>
        );
      }
    }
    return (
      <View style={[styles.centeredContainer, { paddingTop: 25, backgroundColor: '#eee' }]}>
        <Avatar size="large" rounded source={{ uri: this.props.friend.photoURL }} activeOpacity={0.7} />
        ...
        <ScrollView
          contentContainerStyle={{ height: deviceHeight * 0.4, marginTop: 30 }}
        ...
        <TouchableOpacity style={{ padding: 10 }}>
          <Text style={styles.smallRed} onPress={() => this.confirmRemove(this.props.friend)}>
            Remove Friend
          </Text>
        </TouchableOpacity>
      </View>
