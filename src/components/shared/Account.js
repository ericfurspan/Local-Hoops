...
import {
  updateUserStatus,
  updateFriendRequestsReceived,
  updateFriendRequestsSent,
  createFriends,
  cancelFriendRequest,
} from '../../actions/User';

const userStatusTypes = [{ value: 'Looking to hoop' }, { value: 'Available' }, { value: 'Unavailable' }];
...
class Account extends React.Component {
  ...
  denyFriendRequest = (type, prospectiveFriendId) => {
    ...
    if (type === 'received') {
      ...
      const updatedRequests = this.props.currentUser.friendRequestsReceived.filter((request) => {
      ...
      // update state to remove friend request where uid === prospectiveFriendId
      const updatedRequests = this.props.currentUser.friendRequestsSent.filter((request) => {
        return request.uid !== prospectiveFriendId;
      });
  ...
  render() {
    ...
    if (this.props.currentUser) {
        ...
        userStatusPicker = (
          ...
          >
            {userStatusTypes.map((t) => {
              return <Picker.Item key={t.value} label={t.value} value={t.value} color="#333" />;
            })}
          </Picker>
        );
      }
      ...
      if (
        this.props.currentUser.friendRequestsReceived &&
        this.props.currentUser.friendRequestsReceived.length > 0
      ) {
        friendRequestsReceivedBadge = (
          <Badge value={this.props.currentUser.friendRequestsReceived.length} status="error" />
        );
            ...
            <ListItem
              ...
              rightElement={
                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly' }}>
                  ...
                  <TouchableOpacity onPress={() => this.denyFriendRequest('received', pendingFriend.uid)}>
        ...
        friendRequestsSent = this.props.currentUser.friendRequestsSent.map((pendingFriend) => {
          return (
            <ListItem
              ...
              rightElement={
                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly' }}>
                  <TouchableOpacity onPress={() => this.denyFriendRequest('sent', pendingFriend.uid)}>
      ...
      return (
        <ScrollView contentContainerStyle={[styles.container]}>
          ...
          <View style={{ marginBottom: 10, marginTop: 10 }}>
            ...
            >
              <Button
                ...
                icon={{ name: 'users', type: 'font-awesome', size: 25, color: '#333' }}
          ...
          <View style={{ flexDirection: 'row' }}>
            <TouchableOpacity style={{ flexDirection: 'row' }} onPress={() => this.toggleStatusPicker()}>
              <Button
                title="Set your Status"
                titleStyle={{ fontSize: 18, marginLeft: 5 }}
                icon={{ name: 'user-tag', type: 'font-awesome', size: 25, color: '#333' }}
                buttonStyle={{ backgroundColor: 'transparent' }}
                disabled
                disabledStyle={{ backgroundColor: 'transparent' }}
                disabledTitleStyle={{ color: '#333' }}
          ...
          <View style={{ position: 'absolute', bottom: 0, alignSelf: 'center' }}>
            <Button
              ...
              icon={{ name: 'sign-out', type: 'font-awesome', size: 30, color: 'red' }}
              buttonStyle={{ backgroundColor: 'transparent' }}
              testID="Logout"
            />
          </View>

          <Modal transparent={false} visible={this.state.showModal.visible}>
            <View style={styles.modalBackground}>
              <View style={[styles.modalContent]}>
                ...
                />
                <ScrollView>
                  <Text style={[styles.header, { marginTop: 20, marginBottom: 10 }]}>
