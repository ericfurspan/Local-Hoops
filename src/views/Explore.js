...
class Explore extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    const prevCoords = prevProps.location;
    const newCoords = this.props.location;

    // if lat or long has changed, update map center
    if (
      prevCoords &&
      newCoords &&
      (prevCoords.latitude !== newCoords.latitude || prevCoords.longitude !== newCoords.longitude)
    ) {
        ...
        case 'showCourt':
          ...
          this.setState(
            ...
            () => {
              this.props.dispatch(
                getNearbyCourts(
                  { latitude: action.data.coords.latitude, longitude: action.data.coords.longitude },
  ...
  openExternalMap = (coords) => {
    ...
    Linking.openURL(url).catch((err) => console.log('something went wrong opening Apple Map', err));
  };
  ...
  returnAddCourtAnnotation = (coords) => {
    return (
      ...
      // >
      //   <FontAwesome5 name="map-pin" size={40} color="#3578E5" solid />
      // </Mapbox.PointAnnotation>
    );
  };
  ...
  isCourtSaved = (courtId) => {
    const isSaved =
      ...
      this.props.currentUser.saved_courts &&
      this.props.currentUser.saved_courts.includes(courtId);
    return isSaved;
  };

  // Returns a single annotation via an index of nearbyCourts
  returnAnnotation = (counter) => {
    const { coords } = this.props.nearbyCourts[counter];
    ...
    return (
      ...
      // >
      //   <FontAwesome5 name={isSaved ? 'star' : 'map-pin'} size={30} color={isSaved ? 'gold' : 'red'} />
      // </Mapbox.PointAnnotation>
    );
  };
  ...
  renderAnnotations = () => {
    if (this.props.nearbyCourts !== null) {
      const annotations = this.props.nearbyCourts.map((court, i) => {
  ...
  render() {
    ...
    const header = (
      ...
      >
        ...
        />
        <TouchableOpacity
          onPress={() => this.toggleView(this.state.viewMode === 'map' ? 'list' : 'map')}
          testID="toggleExploreViewMode"
        >
          <FontAwesome5
            name={this.state.viewMode === 'map' ? 'list' : 'map-marked'}
            size={30}
            color="white"
          />
        </TouchableOpacity>
          ...
          <View style={[styles.fullCenterContainer, styles.modalContent, { backgroundColor: '#FAFAFA' }]}>
            ...
            >
              ...
              <Button
                ...
                icon={{ name: 'map-marked', type: 'font-awesome', size: 25, color: '#FFFFFF' }}
                backgroundColor="transparent"
    ...
    if (this.state.viewMode === 'list') {
      ...
      const { tempMapType } = this.state;
      const mapStyleUrl = this.returnMapStyleUrl(
        tempMapType || (currentUser && currentUser.preferredMapType)
      );
            ...
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Button
                  ...
                  icon={{ name: 'arrow-left', type: 'font-awesome', size: 30, color: '#3578E5' }}
                ...
                <Button
                  title=""
                  onPress={() => this.toggleAddCourtMode(false)}
                  icon={{ name: 'times-circle', type: 'font-awesome', size: 30, color: '#ccc' }}
                  buttonStyle={{ backgroundColor: 'transparent' }}
              ...
              <View style={{ alignItems: 'center' }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Button
                    ...
                    raised
                    type="outline"
                    disabled={!this.state.addCourtForm.name}
                    ...
                    icon={{
                      ...
                      type: 'font-awesome',
                      size: 18,
                      color: !this.state.addCourtForm.name || this.state.addCourtForm.error ? '#ccc' : '#fff',
                    }}
      ...
      return (
        <View style={[styles.container]} testID="ExploreMapContainer">
          ...
          >
            ...
            >
              <TouchableOpacity style={styles.buttonBr} onPress={() => this.toggleOptionsMenu()}>
                <Icon
                  name="th"
                  type="font-awesome"
              ...
              <TouchableOpacity style={{ width: 45 }} onPress={() => this.updateUserLocation()}>
                <Icon name="gps-fixed" color={this.props.location ? '#3578E5' : 'red'} size={30} />
              </TouchableOpacity>
            </View>
                  ...
                  >
                    <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 15 }}>
                      <Text style={{ fontSize: 15, fontWeight: '500', marginBottom: 10 }}>
                        Missing a court? Help improve the map!
                      </Text>
                      <Button
                        onPress={() => this.toggleAddCourtMode(true)}
                        icon={{ name: 'plus', type: 'font-awesome', size: 18, color: '#fff' }}
                        title="Add Court"
                        raised
                        type="outline"
                        buttonStyle={{ backgroundColor: '#3578E5' }}
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      <TouchableOpacity
                        onPress={() => this.props.dispatch(setPreferredMapType(this.state.tempMapType))}
                      >
                    ...
                    >
                      <TouchableOpacity onPress={() => this.updateTempMapType('Light')}>
                        <Image
                          ...
                          style={[
                            ...
                            (this.state.tempMapType && this.state.tempMapType === 'Light') ||
                            (!this.state.tempMapType && this.props.preferredMapType === 'Light')
                              ? { borderColor: '#3578E5', borderWidth: 2 }
                              : null,
                          ]}
                        />
                      ...
                      <TouchableOpacity onPress={() => this.updateTempMapType('Dark')}>
                        <Image
                          ...
                          style={[
                            { width: 100, height: 100 },
                            (this.state.tempMapType && this.state.tempMapType === 'Dark') ||
                            (!this.state.tempMapType && this.props.preferredMapType === 'Dark')
                              ? { borderColor: '#3578E5', borderWidth: 2 }
                              : null,
                          ]}
                        />
                      ...
                      <TouchableOpacity onPress={() => this.updateTempMapType('Street')}>
                        <Image
                          ...
                          style={[
                            { width: 100, height: 100 },
                            (this.state.tempMapType && this.state.tempMapType === 'Street') ||
                            (!this.state.tempMapType && this.props.preferredMapType === 'Street')
                              ? { borderColor: '#3578E5', borderWidth: 2 }
                              : null,
                          ]}
                        />
              ...
              {this.state.selectedCourtPreview ? (
                ...
                >
                  <Card
                    ...
                    title={
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        <Text style={{ fontSize: 16, fontWeight: 'bold', width: deviceWidth * 0.65 }}>
                          {this.state.selectedCourtPreview.name}
                        </Text>
                    ...
                    <View style={{ justifyContent: 'space-evenly', alignItems: 'flex-start' }}>
                      {this.state.selectedCourtPreview.verified ? (
                        <View style={{ flexDirection: 'row' }}>
                          <Text
                            style={{ color: '#3578E5', fontWeight: 'bold', marginRight: 3, marginTop: 6 }}
                          >
                            Verified
                          </Text>
                          <FontAwesome5 name="check-circle" size={35} color="#3578E5" />
                        </View>
                      ) : null}
                      <Text
                        style={{ marginTop: 10 }}
                      >{`Added by: ${this.state.selectedCourtPreview.discovered_by.displayName}`}</Text>

                      <View style={{ flexDirection: 'row', alignSelf: 'flex-end' }}>
                        <View style={{ alignItems: 'center', marginRight: 15 }}>
                          {this.props.currentUser &&
                          this.props.currentUser.saved_courts &&
                          this.props.currentUser.saved_courts.includes(this.state.selectedCourtPreview.id) ? (
                            <TouchableOpacity onPress={() => this.handleUnSaveCourt()}>
                              <FontAwesome5 name="star" size={30} color="gold" />
                            </TouchableOpacity>
                          ) : (
                            <TouchableOpacity onPress={() => this.handleSaveCourt()}>
                              <FontAwesome5 name="star" size={30} color="gold" />
                            </TouchableOpacity>
                          )}
